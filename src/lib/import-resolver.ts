...
const GRAMMAR_URLS: Record<string, string> = {
  ...
  javascript:
    "https://github.com/tree-sitter/tree-sitter-javascript/releases/latest/download/tree-sitter-javascript.wasm",
  python:
...
export class ImportResolver {
  ...
  async resolveImports(filePath: string): Promise<ResolvedFile> {
    await this.init();
  ...
  async resolveImportsRecursively(
    filePath: string,
    depth: number = 2,
    visited: Set<string> = new Set()
  ): Promise<Map<string, ResolvedFile>> {
    const results = new Map<string, ResolvedFile>();
    ...
    if (depth > 0) {
      for (const imp of fileInfo.imports) {
        if (imp.resolvedPath && !imp.source.includes("node_modules")) {
          const childResults = await this.resolveImportsRecursively(
            imp.resolvedPath,
            depth - 1,
            visited
          );
